#!/usr/bin/env python3
"""
PersonaSystem v2 (frontier-grade) — Stage 1 of Cognitive Loop
Source: ChatGPT recovery chat 15 (advance into frontier-grade territory)

Canonical relationship:
- F:\\Samus iteration (see memory: project_samus_plane_iteration — 4-plane arch)
- [EXPANDS §6 agents.cognitive Stage 1] persona-frame stimulus enrichment
- [EXPANDS §6 autonomy] AutonomyState analog via OperatingMode FSM
- [NEW vs canonical] safety_posture / decision_basis / frame_id observability fields
- [NEW] Emergency drift override (auto-promote to RECOVERY when drift >= 0.9)
- [NEW] Mode hysteresis + hold timer (prevents flapping)
- [NEW] Score-based mode resolution with hysteresis (vs branch-overwrite)

Key upgrades over v1:
  - Thread-safe state (RLock around counters/last_frame/history)
  - Mode hold-timer prevents oscillation (`mode_min_hold_seconds` = 5s)
  - Emergency RECOVERY override at drift >= emergency_drift_recovery_ceiling
  - Deterministic frame_id (sha256 of frame payload, 16-char prefix)
  - safety_posture inference: restrictive | guarded | balanced | expansive
  - Mode-transition audit hooks
  - Safe degradation: bad PersonaMemory snapshot → neutral frame, never crash
"""
...
class PersonaSystem:
    ...
    def __init__(self, persona_memory=None, thresholds=None, enabled=True,
