...
def _p(**kw):
    base = dict(prospect_id="pr_x", company_name="Acme HVAC", industry="dentist",
...
def _live_page(url):
    return {"status_code": 200, "html": "<h1>hi</h1>",
            "final_url": url, "fetch_error": None}
...
def test_recheck_recovers_a_transiently_unreachable_site(monkeypatch):
    ...
    p = _p(website_status="unreachable_timeout", seo_score=0,
           lead_score=84, call_priority="hot")
...
def test_recheck_confirms_a_still_failing_site(monkeypatch):
    ...
    monkeypatch.setattr(crawler, "fetch_homepage", lambda url: {
        "status_code": 0, "html": None, "final_url": url,
        "fetch_error": "timeout: read timed out"})
    monkeypatch.setattr(crawler, "classify_website",
                        lambda page: "unreachable_timeout")
...
def test_recheck_skips_live_and_genuine_no_website(monkeypatch):
    from backend.prospecting import crawler
    calls: list[str] = []
    monkeypatch.setattr(crawler, "fetch_homepage",
                        lambda url: calls.append(url) or _live_page(url))
...
def test_recheck_reclassifies_to_a_different_failure(monkeypatch):
    ...
    from backend.prospecting import crawler
    monkeypatch.setattr(crawler, "fetch_homepage", lambda url: {
        "status_code": 403, "html": None, "final_url": url, "fetch_error": None})
