...
def test_assess_warmth_cold_when_no_collectors_hit(monkeypatch):
    ...
    monkeypatch.setattr(legitimacy_check, "lookup_chamber_roster", lambda name, city: None)
    monkeypatch.setattr(legitimacy_check, "lookup_prior_inbound", lambda **kw: None)
    p = _Prospect(prospect_id="p1", company_name="Acme", city="Yuba City", email="x@y.com")
    a = legitimacy_check.assess_warmth(p)
...
def test_dedupe_by_kind(monkeypatch):
    monkeypatch.setattr(
        legitimacy_check, "lookup_ca_sos", lambda name: _sig(kind="public_registry")
    )
    monkeypatch.setattr(
        legitimacy_check,
        "lookup_chamber_roster",
        lambda name, city: _sig(kind="chamber_roster", confidence="medium"),
    )
    monkeypatch.setattr(
        legitimacy_check, "lookup_prior_inbound", lambda **kw: _sig(kind="public_registry")
    )  # duplicate kind
    p = _Prospect(company_name="Acme", city="Yuba City", email="x@y.com")
...
def test_highest_confidence_kind_prefers_high():
    sigs = [
        _sig(kind="chamber_roster", confidence="medium"),
