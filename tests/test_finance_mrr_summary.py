...
def test_get_mrr_adds_empty_when_log_has_no_subscriptions(tmp_path, monkeypatch):
    ...
    append_event_record(WebhookEventRecord(
        ...
        received_at=_fresh_ts(), livemode=True,
...
def test_get_mrr_adds_counts_and_sums_subscriptions(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import append_event_record
    append_event_record(WebhookEventRecord(
        event_id="sub1", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="a@x.com", amount_total_usd=300.0,
        process_status="processed",
        session_mode="subscription", subscription_id="sub_a",
        subscription_mrr_usd=300.0,
    ))
    append_event_record(WebhookEventRecord(
        event_id="sub2", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="b@x.com", amount_total_usd=125.0,
        process_status="processed",
        session_mode="subscription", subscription_id="sub_b",
...
def test_get_mrr_adds_filters_stale_subscriptions(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import append_event_record
    append_event_record(WebhookEventRecord(
        event_id="sub_fresh", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="fresh@x.com", amount_total_usd=300.0,
        process_status="processed",
        session_mode="subscription", subscription_id="sub_fresh_id",
        subscription_mrr_usd=300.0,
    ))
    append_event_record(WebhookEventRecord(
        event_id="sub_stale", event_type="checkout.session.completed",
        received_at=_stale_ts(30), livemode=True,
        customer_email="stale@x.com", amount_total_usd=300.0,
        process_status="processed",
        session_mode="subscription", subscription_id="sub_stale_id",
        subscription_mrr_usd=300.0,
    ))
    from backend.finance.service import get_mrr_adds
    summary = get_mrr_adds(window_days=7)
...
def test_get_mrr_adds_excludes_non_subscription_events(tmp_path, monkeypatch):
    ...
    append_event_record(WebhookEventRecord(
        event_id="p1", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="p@x.com", amount_total_usd=149.0,
        process_status="processed", session_mode="payment",
    ))
    append_event_record(WebhookEventRecord(
        event_id="ign1", event_type="customer.subscription.updated",
        received_at=_fresh_ts(), livemode=True,
        process_status="ignored",
    ))
    append_event_record(WebhookEventRecord(
        event_id="sub_only", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="s@x.com", amount_total_usd=300.0,
        process_status="processed",
        session_mode="subscription", subscription_id="sub_only_id",
        subscription_mrr_usd=300.0,
    ))
    from backend.finance.service import get_mrr_adds
    summary = get_mrr_adds(window_days=7)
...
def test_get_mrr_adds_excludes_zero_or_missing_mrr(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import append_event_record
    append_event_record(WebhookEventRecord(
        event_id="sub_zero", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="z@x.com", process_status="processed",
        session_mode="subscription", subscription_id="sub_zero_id",
        subscription_mrr_usd=0.0,
    ))
    append_event_record(WebhookEventRecord(
        event_id="sub_none", event_type="checkout.session.completed",
        received_at=_fresh_ts(), livemode=True,
        customer_email="n@x.com", process_status="processed",
        session_mode="subscription", subscription_id="sub_none_id",
