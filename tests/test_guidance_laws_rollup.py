...
class TestGuidanceLawRoundtrip:
    def test_emit_then_read_preserves_fields(self):
        from backend.experiments.promoter import (
            GuidanceLaw, emit_guidance_law, read_guidance_laws,
        )
    ...
    def test_multiple_emits_append_in_order(self):
        ...
        for i in range(3):
            emit_guidance_law(GuidanceLaw(
                law_id=f"law-x-{i}", law=f"pattern {i}",
                evidence_count=i + 1, confidence=0.5,
                promoted_at="2026-07-06T00:00:00Z",
            ))
        laws = read_guidance_laws()
    ...
    def test_read_skips_malformed_rows(self, tmp_path, monkeypatch):
        ...
        from backend.experiments.promoter import (
            _GUIDANCE_LAWS_JSONL, read_guidance_laws,
        )
        ...
        with path.open("w", encoding="utf-8") as fh:
            fh.write(json.dumps({
                "law_id": "law-good", "law": "keep me",
                "evidence_count": 5, "confidence": 0.6,
                "promoted_at": "2026-07-06T00:00:00Z",
                "source_pattern_ids": ["a"], "category": "ops",
            }) + "\n")
            fh.write("not-json\n")
        laws = read_guidance_laws()
    ...
    def test_business_event_emitted(self, monkeypatch):
        ...
        monkeypatch.setattr(
            promoter, "emit_business_event",
            lambda et, **kw: events.append((et, kw)) or {},
        )
        emit_guidance_law(GuidanceLaw(
            law_id="law-evt", law="pattern: event",
...
class TestDistillRollup:
    ...
    def test_evidence_count_parses_fraction_from_recommendation(self):
        ...
        lesson = {
            "recommendation": (
                "pattern: 'dental' industry produced 6/10 closed_won deals"
            ),
    ...
    def test_evidence_count_falls_back_to_impact_floor(self):
        ...
        lesson = {"recommendation": "pattern: qualitative note",
                  "rationale": "provenance: none", "expected_impact": "medium"}
        ev, conf = _evidence_and_confidence(lesson, lesson["rationale"])
        assert ev == 1
        assert conf == 0.5
