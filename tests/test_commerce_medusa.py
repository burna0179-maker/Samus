...
def _settings(**over):
    base = dict(
        commerce_medusa_enabled=True,
        medusa_base_url="https://store.example.com",
        medusa_admin_token="tok",
...
def test_publish_product_creates(monkeypatch):
    client = MedusaClient(base_url="https://store.example.com", admin_token="tok")
    monkeypatch.setattr(
        client, "_request", lambda *a, **k: {"product": {"id": "prod_9", "title": "Mini Lamp"}}
    )
    res = publish_product(
        title="Mini Lamp", price_usd_cents=2999, settings=_settings(), client=client
    )
...
def test_reconcile_orders_attributes_products_stream(monkeypatch):
    ...
    report = reconcile_orders(
        settings=_settings(revenue_entity_products="Forge Goods LLC"), client=client
    )
