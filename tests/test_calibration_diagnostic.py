...
def test_reconcile_empty_stream_returns_scaffold(monkeypatch):
    ...
    out = cd.reconcile_decisions(window_days=7,
                                 until_iso="2026-07-08T00:00:00Z")
    assert out["window"]["days"] == 7
    assert out["overall"] == {
        "decisions": 0, "unscoreable": 0, "loop_completion_rate": 0.0,
    }
...
def test_reconcile_per_bucket_hit_rate_math(monkeypatch):
    ...
    decisions = [
        _decision_event(actor="planner", confidence=0.9,
                        expected_outcome="prospect will be booked",
                        prospect_id="p1"),
        _decision_event(actor="planner", confidence=0.85,
                        expected_outcome="meeting booked this week",
                        prospect_id="p2"),
        _decision_event(actor="planner", confidence=0.95,
...
def test_reconcile_per_actor_breakdown(monkeypatch):
    decisions = [
        _decision_event(actor="planner", confidence=0.9,
                        expected_outcome="booked", prospect_id="p1"),
        _decision_event(actor="arbiter", confidence=0.5,
                        expected_outcome="paid", opportunity_id="o1"),
    ]
    outcomes = [
        _outcome_event(event_type="meeting.booked", prospect_id="p1"),
...
def test_loop_completion_rate_counts_any_outcome(monkeypatch):
    ...
    decisions = [
        _decision_event(actor="planner", confidence=0.7,
                        expected_outcome="booked", prospect_id="p1",
                        decision_id="d1"),
        _decision_event(actor="planner", confidence=0.7,
                        expected_outcome="reply", prospect_id="p1",
                        decision_id="d2"),
        _decision_event(actor="planner", confidence=0.7,
                        expected_outcome="booked", prospect_id="p_dark",
                        decision_id="d3"),
    ]
    outcomes = [
    ...
    assert out["per_actor"]["planner"]["loop_completion_rate"] == pytest.approx(
        2 / 3, abs=1e-4
    )
...
def test_unscoreable_fraction_reported(monkeypatch):
    ...
    decisions = [
        _decision_event(actor="planner", confidence=0.9,
                        expected_outcome="prospect booked", prospect_id="p1"),
        _decision_event(actor="planner", confidence=0.9,
                        expected_outcome="???",
                        prospect_id="p2"),
    ]
    ...
    # not to hits.
    top = [b for b in out["per_actor"]["planner"]["calibration"]
           if b["bucket"] == "0.8-1.0"][0]
    assert top["count"] == 2
    assert top["hits"] == 1
...
def test_return_shape_is_stable(monkeypatch):
    ...
    assert set(out["overall"].keys()) == {
        "decisions", "unscoreable", "loop_completion_rate",
    }
