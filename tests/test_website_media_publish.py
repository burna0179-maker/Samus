            ...
            return httpx.Response(
                ...
                json={
                    "file": {
                        ...
                        "mediaType": "video",
                        "mimeType": "video/mp4",
                        "wixUrl": "wix:video://v1/vid-1/",
                        "url": "https://video.wixstatic.com/vid-1.mp4",
                    }
                },
            )
...
def _video_asset():
    return MediaAsset(
        ...
        ok=True,
        mime_type="video/mp4",
        video_uri="https://veo.test/clip.mp4",
    )
...
def test_upload_bytes_generates_url_then_puts():
    rec = []
    desc = _wix(rec).upload_bytes(
        site_id="s1", data=b"PNGBYTES", filename="hero.png", mime_type="image/png"
    )
...
def test_import_file_posts_url():
    rec = []
    desc = _wix(rec).import_file(
        site_id="s1", url="https://veo.test/clip.mp4", mime_type="video/mp4", display_name="hero"
    )
