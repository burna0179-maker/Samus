...
def test_register_roundtrip_and_hash_provenance(artifact_root):
    ...
    entry = register_asset("pr_acme", data, "logo", original_name="logo.png",
                           notes="operator drop")
...
def test_gate_remote_allowlist_and_strict_mode(artifact_root):
    ...
    clean, violations = enforce_asset_isolation(
        files, "pr_b", own_domains=("demo-x.pages.dev",))
...
def _rec(pid: str, name: str):
    ...
    return ProspectRecord(
        prospect_id=pid, company_name=name, phone="555-0000",
        industry="plumbing", city="Yuba City", state="CA",
        lead_score=75, website_status="no_website",
...
def test_demo_sites_per_client_assets_and_gate(artifact_root, monkeypatch):
    ...
    settings = SimpleNamespace(
        website_demo_sites_enabled=True, anthropic_api_key="",
        cloudflare_api_token="", cloudflare_account_id="",
        demo_sites_ceiling_usd=5.0, website_asset_isolation_strict=False,
    )
    run = build_demo_sites(
        [_rec("pr_A", "Acme Plumbing"), _rec("pr_B", "Bravo Plumbing")],
        deploy=False, use_llm_copy=False, settings=settings, run_date=RUN_DATE,
    )
