...
from backend.tiktok.campaigns import build_caption, build_product_reel, post_to_tiktok
...
def _settings(**over):
    base = dict(
        tiktok_research_provider="none",
        tiktok_research_api_key="",
        tiktok_research_base_url="",
        tiktok_content_posting_enabled=False,
...
def test_research_parses_provider_results(monkeypatch):
    ...
    out = find_trending(
        "home", settings=_settings(tiktok_research_provider="netrows", tiktok_research_api_key="k")
    )
    assert len(out) == 1
...
def test_build_product_reel_success(monkeypatch):
    ...
    res = build_product_reel(
        TrendingProduct(title="Mini Lamp", category="home"), settings=_settings()
    )
...
def test_post_to_tiktok_live_requires_token(monkeypatch):
    ...
    result = CampaignResult(ok=True, product_title="X", reel_path="/tmp/r.mp4", caption="c")
    out = post_to_tiktok(
        result, settings=_settings(tiktok_dry_run=False, tiktok_content_posting_enabled=True)
    )
...
def test_seller_disabled_is_dormant():
    ...
    assert (
        shop_seller.create_listing(
            title="X", description="d", price_usd=9.99, settings=_settings()
        )["ok"]
        is False
    )
