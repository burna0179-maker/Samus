...
def test_aggregate_rates():
    daily = _stats(requests=100, delivered=95, bounces=3, blocks=1,
                   spam_reports=0, unique_opens=38, unique_clicks=5, unsubscribes=1)
...
def test_reputation_alert_spam_is_critical():
    m = eba.aggregate(_stats(requests=100, delivered=100, spam_reports=1),
...
def test_reputation_alert_high_bounce_is_warning():
    m = eba.aggregate(_stats(requests=100, delivered=90, bounces=8),
                      window_start="a", window_end="b")
    alerts = eba.reputation_alerts(m)
...
def test_reputation_healthy_no_alerts():
    m = eba.aggregate(_stats(requests=100, delivered=99, bounces=1),
                      window_start="a", window_end="b")
