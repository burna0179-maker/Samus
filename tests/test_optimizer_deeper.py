...
def test_apply_reward_compound():
    ...
    weights = RewardWeights(reply_weight=1.0, conversion_weight=5.0,
...
def test_score_opportunity_increases_with_expected_value():
    ...
    lo = Opportunity(prospect_id="lo", expected_value=1_000,
                     conversion_prob=0.3, execution_cost=50)
    hi = Opportunity(prospect_id="hi", expected_value=2_000,
                     conversion_prob=0.3, execution_cost=50)
...
def test_score_opportunity_clamped_above_zero():
    from backend.optimizer.models import Opportunity
    from backend.optimizer.portfolio import score_opportunity
    opp = Opportunity(prospect_id="boom", expected_value=10.0,
                      conversion_prob=0.01, execution_cost=10_000.0)
...
def test_momentum_from_signals_cap_at_one():
    ...
    assert momentum_from_signals([
        "email_open", "link_click", "pricing_request",
        "reply", "demo_request",
    ]) == 1.0
...
def test_optimize_portfolio_under_budget(tmp_path, monkeypatch):
    ...
    opps = [
        Opportunity(prospect_id=f"p{i}", expected_value=10_000,
                    conversion_prob=0.5, execution_cost=100.0,
                    time_to_close=1)
        for i in range(3)
    ]
    result = optimize_portfolio(OptimizePortfolioRequest(
        campaign_id="under", total_budget=1000.0, opportunities=opps,
    ))
...
def test_optimize_portfolio_over_budget_demotes_to_defer(tmp_path, monkeypatch):
    ...
    opps = [
        Opportunity(prospect_id=f"p{i}", expected_value=10_000,
                    conversion_prob=0.5, execution_cost=200.0,
                    time_to_close=1)
        for i in range(10)
    ]
    result = optimize_portfolio(OptimizePortfolioRequest(
        campaign_id="over", total_budget=300.0, opportunities=opps,
    ))
...
def test_optimize_portfolio_uses_signals_for_momentum_when_zero(tmp_path, monkeypatch):
    ...
    from backend.optimizer.service import optimize_portfolio
    op_with = Opportunity(
        prospect_id="with_sigs", expected_value=10_000, conversion_prob=0.5,
        execution_cost=10.0, time_to_close=1, momentum=0.0,
        signals=["email_open", "link_click"],
    )
    op_without = Opportunity(
        prospect_id="no_sigs", expected_value=10_000, conversion_prob=0.5,
        execution_cost=10.0, time_to_close=1, momentum=0.0,
    )
    result = optimize_portfolio(OptimizePortfolioRequest(
        campaign_id="sig", total_budget=10_000.0,
...
def test_update_arm_endpoint_happy_path(tmp_path, monkeypatch):
    ...
    from backend.optimizer.app import app
    client = TestClient(app)
    client.post("/select_arm", json={
        "campaign_id": "c-upd", "arms": ["a", "b"],
    })
    r = client.post("/update_arm", json={
        "campaign_id": "c-upd",
        "arm": "a",
...
def test_optimize_portfolio_endpoint_happy_path(tmp_path, monkeypatch):
    ...
    from backend.optimizer.app import app
    client = TestClient(app)
    r = client.post("/optimize_portfolio", json={
        "campaign_id": "c-opt",
        "total_budget": 5_000.0,
        "opportunities": [
            {"prospect_id": f"p{i}", "expected_value": 10_000,
             "conversion_prob": 0.5, "execution_cost": 100.0,
             "time_to_close": 1}
            for i in range(5)
        ],
