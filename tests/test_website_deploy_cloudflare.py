...
def _site():
    return GeneratedSite(files={"index.html": "<html><body>Sample Cleaning</body></html>"},
                         taste_audit={"passed": True, "grade": "A"}, pages=["index.html"])


def _settings(**over):
    base = dict(
        website_deploy_enabled=True, website_deploy_host="cloudflare",
        cloudflare_api_token="cf-token", cloudflare_account_id="acct-1",
...
def test_deploy_via_wrangler_parses_url(monkeypatch, tmp_path):
    ...
    res = dc.deploy_via_wrangler(tmp_path, project="sample-cleaning",
                                 api_token="cf-token", account_id="acct-1")
...
def test_deploy_via_wrangler_nonzero_exit_fails(monkeypatch, tmp_path):
    monkeypatch.setattr(dc, "_run", lambda *a, **k: SimpleNamespace(returncode=1, stdout="", stderr="auth error"))
...
def test_deploy_site_runs_when_armed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(dc, "_run", lambda *a, **k: SimpleNamespace(
