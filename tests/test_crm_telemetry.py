...
def test_latency_helper_computes_seconds():
    ...
    latency = latency_to_resolution_sec(
        "2026-05-20T00:00:00Z", "2026-05-20T01:00:00Z",
    )
...
def test_latency_helper_none_on_negative_delta():
    ...
    assert latency_to_resolution_sec(
        "2026-05-20T05:00:00Z", "2026-05-20T01:00:00Z",
    ) is None
...
def test_advance_to_terminal_surfaces_latency(tmp_path, monkeypatch):
    ...
    tables["_opportunities_table"].items[("opportunity_id", "op_lat")] = {
        "opportunity_id": "op_lat", "stage": "proposal",
        "deal_size_usd": 500.0,
    ...
    result = advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_lat", target_stage="closed_won",
...
def test_non_terminal_advance_has_no_latency(tmp_path, monkeypatch):
    ...
    tables["_opportunities_table"].items[("opportunity_id", "op_nt")] = {
        "opportunity_id": "op_nt", "stage": "new",
        "created_at": "2026-05-19T00:00:00Z",
    }
    from backend.crm.service import advance_opportunity
    from backend.crm.models import AdvanceOpportunityRequest
    ...
    result = advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_nt", target_stage="qualified",
    ))
    assert result.status == "advanced"
...
def test_latency_lands_in_audit_event(tmp_path, monkeypatch):
    ...
    tables["_opportunities_table"].items[("opportunity_id", "op_aud")] = {
        "opportunity_id": "op_aud", "stage": "negotiation",
        "created_at": "2026-05-18T00:00:00Z", "deal_size_usd": 900.0,
    }
    from backend.crm.service import advance_opportunity
    from backend.crm.models import AdvanceOpportunityRequest

    advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_aud", target_stage="closed_lost",
        lost_reason="ghosted",
    ))
    lines = [json.loads(l) for l in audit_path.read_text(encoding="utf-8").splitlines() if l.strip()]
    advanced = [r for r in lines if r.get("action") == "advance_opportunity"
...
def test_token_cost_rollup_per_industry(monkeypatch):
    tables = _patch_tables(monkeypatch)
    _seed_opportunities(tables, [
        {"opportunity_id": "o1", "industry": "plumbing", "stage": "new",
         "token_cost_usd": 0.10},
        {"opportunity_id": "o2", "industry": "plumbing", "stage": "new",
         "token_cost_usd": 0.30},
        {"opportunity_id": "o3", "industry": "hvac", "stage": "new",
         "token_cost_usd": 0.05},
    ])
...
def test_token_cost_route(monkeypatch):
    ...
    tables = _patch_tables(monkeypatch)
    _seed_opportunities(tables, [
        {"opportunity_id": "o1", "industry": "roofing", "stage": "new",
         "token_cost_usd": 0.25},
    ])
    from backend.crm.app import app
