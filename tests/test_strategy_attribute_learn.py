...
def test_log_call_booked_forwards_arm_into_opportunity_request(tmp_path, monkeypatch):
    ...
    def _create_opportunity(req):
        ...
        return CreateOpportunityResult(
            status="created", opportunity_id="op_booked",
    ...
    result = log_call(
        ...
        notes="booked an audit", industry="hvac",
        policy_family="fast_quote_mode", seo_score=58,
        owner_email=True, social_facebook=False, social_instagram=True,
    )
...
def test_log_call_non_booked_does_not_create_opportunity(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(crm_service, "upsert_call_state", lambda s: True)
    monkeypatch.setattr(crm_service, "get_call_state", lambda pid: None)
    monkeypatch.setattr(crm_service, "create_opportunity",
                        lambda req: created.append(req))

    from backend.crm.log_call import log_call
    result = log_call(
        prospect_id="pr_acme", outcome="follow_up", industry="hvac",
        policy_family="fast_quote_mode",
    )
...
def test_terminal_closed_won_dispatches_outcome_to_strategy(tmp_path, monkeypatch):
    ...
    _seed_opportunity(
        tables, op_id="op_w", stage="negotiation", prospect_id="pr_acme",
        industry="hvac", policy_family="fast_quote_mode", seo_score=70,
        owner_email=True, social_facebook=True, social_instagram=False,
    )
...
def test_terminal_closed_lost_dispatches_zero_outcome(tmp_path, monkeypatch):
    ...
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(
        tables, op_id="op_l", stage="proposal", prospect_id="pr_acme",
    ...
    from backend.crm.models import AdvanceOpportunityRequest
    result = advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_l", target_stage="closed_lost", lost_reason="ghosted",
    ))
    assert result.status == "advanced"
    assert len(posts) == 1
...
def test_non_terminal_transition_does_not_dispatch(tmp_path, monkeypatch):
    ...
    _audit_to_tmp(monkeypatch, tmp_path)
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(tables, op_id="op_q", stage="new",
    ...
    from backend.crm.models import AdvanceOpportunityRequest
    result = advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_q", target_stage="qualified",
    ))
    assert result.status == "advanced"
    assert posts == []
...
def test_dispatch_is_best_effort_strategy_outage_does_not_fail_write(
    ...
    _audit_to_tmp(monkeypatch, tmp_path)
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(tables, op_id="op_w2", stage="negotiation",
                      industry="hvac", policy_family="fast_quote_mode")
    _stub_strategy_settings(monkeypatch)
...
def test_dispatch_skipped_when_strategy_url_unset(tmp_path, monkeypatch):
    ...
    _audit_to_tmp(monkeypatch, tmp_path)
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(tables, op_id="op_w3", stage="negotiation",
                      industry="hvac", policy_family="fast_quote_mode")
    ...
    from backend.crm.models import AdvanceOpportunityRequest
    result = advance_opportunity(AdvanceOpportunityRequest(
        opportunity_id="op_w3", target_stage="closed_won",
    ))
    assert result.status == "advanced"
...
def test_close_from_payment_routes_through_single_dispatch_point(
    ...
    _audit_to_tmp(monkeypatch, tmp_path)
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(tables, op_id="op_pay", stage="negotiation",
                      industry="roofing", policy_family="storm_damage_mode")
    _stub_strategy_settings(monkeypatch)
    posts = _capture_strategy_posts(monkeypatch)
...
def test_advance_with_lifecycle_routes_through_single_dispatch_point(
    ...
    _audit_to_tmp(monkeypatch, tmp_path)
    tables = _patch_tables(monkeypatch)
    _seed_opportunity(tables, op_id="op_lc", stage="proposal",
                      industry="hvac", policy_family="fast_quote_mode")
    _stub_strategy_settings(monkeypatch)
    posts = _capture_strategy_posts(monkeypatch)
    ...
    from backend.crm.models import AdvanceOpportunityRequest
    result = advance_opportunity_with_lifecycle(AdvanceOpportunityRequest(
        opportunity_id="op_lc", target_stage="closed_won",
    ))
    assert result.status == "advanced"
    assert len(posts) == 1
...
def test_record_outcome_with_arm_builds_reward_signal_and_updates_bandit(monkeypatch):
    ...
    req = RecordOutcomeRequest(
        prospect_id="pr_acme", won=True,
        industry="hvac", policy_family="fast_quote_mode",
        outcome=1.0, seo_score=80,
        owner_email=True, social_facebook=True, social_instagram=False,
    )
...
def test_record_outcome_without_owner_email_uses_neutral_contactability(monkeypatch):
    ...
    from backend.strategy.models import RecordOutcomeRequest
    req = RecordOutcomeRequest(
        prospect_id="pr_x", won=False,
        industry="dentist", policy_family="reputation_repair",
        outcome=0.0, seo_score=20, owner_email=False,
    )
...
def test_record_outcome_partial_arm_skips_bandit(monkeypatch):
    ...
    from backend.strategy.models import RecordOutcomeRequest
    req = RecordOutcomeRequest(
        prospect_id="pr_partial", won=True, industry="hvac", policy_family="",
    )
    asyncio.run(_record(req))
    assert calls == []
...
def test_record_outcome_real_bandit_records_trial(monkeypatch):
    ...
    req = RecordOutcomeRequest(
        prospect_id="pr_acme", won=True,
        industry="hvac", policy_family="fast_quote_mode",
        outcome=1.0, seo_score=50, owner_email=True,
    )
    asyncio.run(_record(req))
