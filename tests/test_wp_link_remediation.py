...
from backend.catalog import wp_link_remediation as rem
from backend.catalog.wp_link_remediation import (
    LinkFix, RemediationPlan, apply_remediations, plan_remediations,
)
...
def _stripe_and_wp_mock(*, wp_pages, stripe_links):
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if "api.stripe.com" in url:
            data = [
                {"id": f"pl_{i}", "url": u, "active": a,
                 "line_items": {"data": [
                     {"amount_total": amt, "description": desc}]}}
...
def test_plan_resolves_unique_successor():
    pages = [{"id": 300, "slug": "workflow-rescue", "title": "Rescue",
...
def test_plan_test_placeholder_is_manual():
    pages = [{"id": 248, "slug": "home", "title": "Home",
              "content": f'<a href="{TEST_URL}">Buy</a>'}]
    with _stripe_and_wp_mock(wp_pages=pages, stripe_links=_DEFAULT_STRIPE) as c:
        plan = plan_remediations(api_key="sk_test", http_client=c)
    assert plan.auto == []
...
def _fix(page_id=300, stale=STALE, live=LIVE):
    return LinkFix(page_id=page_id, post_type="page", slug="s", title="T",
                   stale_url=stale, live_url=live, action="replace", reason="")
...
def test_apply_armed_replaces_url(monkeypatch):
    ...
    monkeypatch.setattr(wp, "get_page_raw",
                        lambda pid: {"id": pid, "content": {"raw": raw}})
...
def test_apply_two_fixes_one_page_single_write(monkeypatch):
    ...
    monkeypatch.setattr(wp, "update_page_content",
                        lambda pid, content: writes.append((pid, content)) or {"id": pid})

    plan = RemediationPlan(fixes=[
        _fix(), _fix(stale=stale2, live=live2),
    ])
...
def test_apply_stale_url_missing_from_raw_is_failed(monkeypatch):
    ...
    from backend.common import wordpress_client as wp
    monkeypatch.setattr(wp, "get_page_raw",
                        lambda pid: {"id": pid, "content": {"raw": "<p>edited meanwhile</p>"}})
    called = []
    monkeypatch.setattr(wp, "update_page_content",
                        lambda pid, content: called.append(pid))
...
def test_apply_manual_fixes_never_written(monkeypatch):
    ...
    manual = LinkFix(page_id=1, post_type="page", slug="s", title="T",
                     stale_url=TEST_URL, live_url="", action="manual", reason="r")
