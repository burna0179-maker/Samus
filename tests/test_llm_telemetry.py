...
def _telemetry_to_tmp(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SAMUS_LLM_TELEMETRY_PATH", str(tmp_path / "llm_calls.jsonl"),
    )
...
class TestRecordLlmCall:
    def test_routed_record_is_reconstructable(self):
        llm_telemetry.record_llm_call(
            workcell="cognition", backend=llm_telemetry.BACKEND_LOCAL,
            decision=llm_telemetry.ROUTED, model="local", est_tokens=1775,
            used_tokens=150, quota_tokens=200_000, actual_cost_usd=0.0,
            outcome="success", latency_ms=42.0,
        )
    ...
    def test_denied_record_carries_control(self):
        llm_telemetry.record_llm_call(
            workcell="cognition", backend=llm_telemetry.BACKEND_OPENAI,
            decision=llm_telemetry.DENIED, model="gpt-4.1-mini",
            control=llm_telemetry.CONTROL_WORKCELL_QUOTA,
            reason="budget_exceeded: used=199518 + req=1775 > quota=200000",
            est_tokens=1775, used_tokens=199_518, quota_tokens=200_000,
        )
        denied = llm_telemetry.read_calls(
            workcell="cognition", decision="denied",
        )
        assert len(denied) == 1
    ...
    def test_read_calls_filters_by_backend(self):
        llm_telemetry.record_llm_call(
            workcell="cognition", backend=llm_telemetry.BACKEND_OPENAI,
            decision=llm_telemetry.ROUTED, outcome="success",
        )
        llm_telemetry.record_llm_call(
            workcell="cognition", backend=llm_telemetry.BACKEND_LOCAL,
            decision=llm_telemetry.ROUTED, outcome="success",
    ...
    def test_reason_is_bounded(self):
        rec = llm_telemetry.record_llm_call(
            workcell="x", backend="local", decision=llm_telemetry.DENIED,
            reason="z" * 5000,
        )
...
class TestFailSoft:
    def test_ledger_error_never_raises(self, monkeypatch):
        ...
        rec = llm_telemetry.record_llm_call(
            workcell="cognition", backend="local",
            decision=llm_telemetry.ROUTED, outcome="success",
        )
...
class TestCounters:
    def test_routing_and_denial_counters_increment(self):
        routed_before = metrics.SAMUS_LLM_ROUTING_TOTAL.labels(
            workcell="countcell", backend="local",
        )._value.get()
        denied_before = metrics.SAMUS_LLM_DENIALS_TOTAL.labels(
            workcell="countcell", control="workcell_quota",
        )._value.get()
        ...
        assert metrics.SAMUS_LLM_ROUTING_TOTAL.labels(
            workcell="countcell", backend="local",
        )._value.get() == routed_before + 1
        assert metrics.SAMUS_LLM_DENIALS_TOTAL.labels(
            workcell="countcell", control="workcell_quota",
        )._value.get() == denied_before + 1
...
class TestLlmClientEmitsTrace:
    def test_success_emits_routed_trace(self, tmp_path, monkeypatch):
        ...
        text, usage = llm_client.anthropic_messages(
            workcell="cognition", api_key="k", prompt="hi", store=store,
        )
    ...
    def test_over_quota_emits_denied_trace(self, tmp_path, monkeypatch):
        ...
        store.record_spend(
            "cognition", input_tokens=8, output_tokens=3, outcome="success",
        )
        with pytest.raises(llm_client.BudgetExceeded):
            llm_client.anthropic_messages(
                workcell="cognition", api_key="k", prompt="x" * 1000,
                store=store, max_tokens=500,
            )
        denied = llm_telemetry.read_calls(
            workcell="cognition", decision="denied",
        )
    ...
    def test_broker_denied_emits_broker_trace(self, tmp_path, monkeypatch):
        ...
        def _deny(**kwargs):
            raise llm_client._broker_client.BrokerDenied(
                kind="llm_tokens", reason="capacity_exhausted",
                retry_after_sec=30,
            )
