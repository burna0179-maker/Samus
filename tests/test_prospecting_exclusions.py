...
def test_post_office_and_police_are_excluded():
    ...
    assert exclusion_reason(
        ...
        website_url="https://usps.com", company_name="US Post Office",
    ) == "government_office"
    assert exclusion_reason(
        place_types="police", website_url="", company_name="City PD",
    ) == "government_office"
...
def test_denylist_matches_www_bare_and_subdomain():
    from backend.prospecting.exclusions import exclusion_reason
    for url in ("https://amplahealth.org", "amplahealth.org",
                "https://www.amplahealth.org/contact",
                "https://clinic.amplahealth.org/about"):
        assert exclusion_reason(
            place_types="doctor", website_url=url, company_name="x",
        ) == "denylist_domain:amplahealth.org", url
...
def test_name_substring_denylist(monkeypatch):
    ...
    reason = ex.exclusion_reason(
        place_types="establishment", website_url="https://x.com",
