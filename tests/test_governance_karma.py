...
def test_penalty_lowers_dimension_and_audits(karma_env, monkeypatch):
    ...
    audited: list = []
    monkeypatch.setattr("backend.common.audit.record", lambda et, **kw: audited.append((et, kw)))
...
def test_gate_blocks_on_low_karma_when_enabled(karma_env, monkeypatch):
    ...
    monkeypatch.setattr(gate_mod, "check_action", lambda action: type("V", (), {"allowed": True})())
