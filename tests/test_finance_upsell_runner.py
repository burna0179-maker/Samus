...
def test_process_returns_zero_when_no_rows_are_due():
    ...
    result = process_upsell_queue(
        now=delivered_at + timedelta(days=1), send_email_fn=fake_send,
    )
    assert result.due == 0
    assert sent_calls == []
...
def test_process_sends_one_email_per_due_row_and_marks_sent():
    ...
    result = process_upsell_queue(
        now=delivered_at + timedelta(days=40), send_email_fn=fake_send,
    )
    assert result.due == 3
...
def test_process_failed_rows_do_not_replay_on_second_pass():
    ...
    second = process_upsell_queue(
        now=delivered_at + timedelta(days=41), send_email_fn=fake_send_ok,
    )
    assert second.due == 0
...
def test_process_falls_back_to_failed_on_no_composer():
    ...
    _append(UpsellQueueRow(
        event_id="e1", ts="2026-05-15T00:00:00Z", kind="queued",
        touch_num=1, customer_id="c", customer_email="c@x.com",
        source_offer_code="unmapped_offer", target_offer_code="not_a_target",
