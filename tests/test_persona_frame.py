...
class _StubMemory:
    ...
    def __init__(self, *, valence=0.0, confidence=0.5, novelty=0.0, drift=0.0,
                 emotion_events=0, reflection_events=0):
        self._snap = {
            "emotion": {"valence": valence, "novelty": novelty, "drift": drift},
            "baseline_confidence": confidence,
            "counters": {"emotion_events": emotion_events,
...
def test_normal_baseline(monkeypatch):
    ...
    sys_ = PersonaSystem(persona_memory=_StubMemory(valence=0.0, confidence=0.5, drift=0.1),
                         enabled=True)
    assert _mode_of(sys_.transform({})) == OperatingMode.NORMAL.value
...
def test_explore_when_conditions_favorable(monkeypatch):
    _set_flag(monkeypatch, True)
    mem = _StubMemory(valence=0.5, confidence=0.85, novelty=0.1, drift=0.05,
                      emotion_events=8, reflection_events=8)  # n_history=16 >= 10
...
def test_cautious_on_high_drift(monkeypatch):
    _set_flag(monkeypatch, True)
    sys_ = PersonaSystem(persona_memory=_StubMemory(confidence=0.5, drift=0.7),
                         enabled=True)
    out = sys_.transform({})
...
def test_recovery_on_multi_signal(monkeypatch):
    ...
    sys_ = PersonaSystem(persona_memory=_StubMemory(valence=-0.5, confidence=0.2),
                         enabled=True)
    out = sys_.transform({})
...
def test_recovery_on_error_streak(monkeypatch):
    _set_flag(monkeypatch, True)
    sys_ = PersonaSystem(persona_memory=_StubMemory(valence=0.0, confidence=0.6),
                         enabled=True)
...
def test_emergency_drift_forces_recovery(monkeypatch):
    ...
    sys_ = PersonaSystem(persona_memory=_StubMemory(valence=0.9, confidence=0.95, drift=0.95),
                         enabled=True)
    out = sys_.transform({})
    assert _mode_of(out) == OperatingMode.RECOVERY.value
...
def test_frame_id_is_deterministic_and_snapshot_shape(monkeypatch):
    ...
    snap = sys_.snapshot()
    assert set(snap) >= {"enabled", "mode", "consecutive_errors", "frame",
                         "mode_history", "thresholds"}
