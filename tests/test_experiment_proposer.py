...
def _patch_belief_reads(
    ...
    *,
    ...
    def _list_all(*, status: str | None = None):
        return [
            _FakeBelief(
                belief_id=f"settled-{i}",
                claim=c,
                confidence=0.95,
                economic_impact=100.0,
                tier=2,
...
def _patch_approvals(monkeypatch) -> list[dict[str, Any]]:
    ...
    def _fake_create(
        kind, payload=None, *, risk_level="normal", ev_usd=None, confidence=None, ttl_seconds=None
    ):
        row = {
...
class TestHappyPath:
    def test_low_risk_proposal_is_promoted(self, monkeypatch):
        ...
        active = [e for e in registry.list_experiments(status="active") if e.dimension == "opener"]
...
class TestSafetyGate:
    def test_high_risk_proposal_blocks_on_hotl(self, monkeypatch):
        ...
        active_pricing = [
            e for e in registry.list_experiments(status="active") if e.dimension == "pricing_tier"
        ]
