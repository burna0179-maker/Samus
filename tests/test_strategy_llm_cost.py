...
def test_callsheet_llm_cost_captured_on_prospect(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: [
            ProspectRecord(
                prospect_id="pr_cs",
                company_name="Callsheet Co",
                website_url="https://cs.example",
                industry="hvac",
                zipcode=kw["zipcode"],
            )
        ],
    ...
    monkeypatch.setattr(
        cs,
        "get_settings",
        lambda: type("S", (), {"anthropic_api_key": "sk-x"})(),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: [
            ProspectRecord(
                ...
                website_url="https://both.example",
                industry="roofing",
                zipcode=kw["zipcode"],
            )
        ],
...
def test_llm_cost_zero_on_templated_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: [
            ProspectRecord(
                prospect_id="pr_tpl",
                company_name="Templated Co",
                website_url="https://tpl.example",
                industry="hvac",
                zipcode=kw["zipcode"],
            )
        ],
    ...
    monkeypatch.setattr(
        cs,
        "get_settings",
        lambda: type("S", (), {"anthropic_api_key": ""})(),
    )
...
def test_callsheet_llm_toggle_off_fires_no_callsheet_llm_but_audit_cost_flows(
    ...
    def _must_not_fire(*a, **k):
        raise AssertionError("callsheet LLM fired with enable_llm_callsheet=False")
