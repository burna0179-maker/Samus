...
class TestBusinessEventsShim:
    ...
    def test_passthrough_when_module_present(self, monkeypatch):
        ...
        out = shim.emit_business_event(
            "experiment.assigned", workcell="experiments",
            prospect_id="p1", variant_arm_id="exp::e1::a",
        )
...
class TestLifecycleRewardHook:
    def test_terminal_transition_triggers_compute_reward(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "backend.strategy.reward_density.compute_reward", fake_compute,
        )
        tasks = lifecycle.tasks_for_stage_advance(
            _opp("closed_won"), "proposal", "closed_won",
        )
...
class TestArmStamp:
    ...
    def test_record_and_lookup_roundtrip(self, state_root):
        ...
        ok = arm_stamp.record_dispatch(
            call_id="call-1", prospect_id="p-9",
            phone="+1 (530) 555-1234", arm_id="voice::a::v1",
        )
        assert ok is True
...
class TestTranscriptRewardFlow:
    def _analysis(self, **over):
        ...
        base = dict(
            source_file="t.txt", file_hash="h1", call_ts_iso="2026-07-05T00:00:00",
            direction="outbound", contact_phone="+15305551234",
            contact_name="Kelly", prospect_id="p-9", company_name="Acme",
            outcome="converted", reward=1.0,
        )
        base.update(over)
    ...
    def test_reward_flows_to_bandit(self, state_root, monkeypatch):
        ...
        arm_stamp.record_dispatch(
            call_id="c1", prospect_id="p-9", phone="+15305551234",
            arm_id="voice::a::v1",
        )
        recorded: list[tuple] = []
        monkeypatch.setattr(
...
class TestOutreachInteractionLedger:
    ...
    def _isolate(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SAMUS_LEDGER_BACKEND", "jsonl")
        monkeypatch.setenv(
            "SAMUS_FEEDBACK_STORE_PATH", str(tmp_path / "metrics.json"),
        )
