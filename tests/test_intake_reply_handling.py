...
def test_interested_draft_is_compliant_and_recommended(draft_env):
    d = drafter.draft_follow_up(
        rc.INTENT_INTERESTED, original_subject="quick question", from_addr="lead@y.com",
        company="Acme",
    )
...
def _parsed(subject: str, body: str, from_addr: str = "lead@y.com"):
    ...
    return ParsedInboundEmail(
        message_id="m1", from_addr=from_addr, from_display="Lead",
        to_addrs=["samus@hustleforge.tech"], subject=subject,
        date_header="", body_text=body, body_format="text", attachment_names=[],
    )
...
def wiring_env(draft_env, monkeypatch):
    ...
    sup: list = []
    arts: list = []
    monkeypatch.setattr("backend.feedback.handlers.fire_cash_engine_signal",
                        lambda **kw: signals.append(kw) or {"ok": True})
    monkeypatch.setattr("backend.common.recipient_index.lookup_recipient",
    ...
    monkeypatch.setattr("backend.crm.service.create_artifact",
                        lambda req: arts.append(req) or type("R", (), {"artifact_id": "a1", "status": "created", "error": None})())
