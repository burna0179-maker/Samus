...
def test_module_constants_match_taxonomy():
    ...
    for const in (
        ...
        be.INVOICE_SENT,
        be.PAYMENT_RECEIVED,
        be.CUSTOMER_RETAINED,
        be.CUSTOMER_CHURNED,
        be.DECISION_MADE,
        be.EXPERIMENT_ASSIGNED,
        be.LAW_PROMOTED,
    ):
...
def _seed(ledger_path):
    be.emit_business_event(be.LEAD_CREATED, workcell="intake", metadata={"lead_id": "l1"})
    be.emit_business_event(be.LEAD_ENRICHED, workcell="prospecting", prospect_id="p1")
    be.emit_business_event(be.EMAIL_SENT, workcell="outreach", prospect_id="p1", campaign_id="c1")
    be.emit_business_event(be.CALL_PLACED, workcell="voice", prospect_id="p2")
    be.emit_business_event(
        be.PAYMENT_RECEIVED,
        workcell="finance",
        prospect_id="p1",
        opportunity_id="op_9",
...
def test_read_limit_keeps_most_recent(ledger_path):
    for i in range(5):
        be.emit_business_event(
            be.EMAIL_SENT, workcell="outreach", prospect_id="p1", metadata={"n": i}
        )
...
def test_conversion_record_accepts_and_persists_campaign_id(tmp_path, monkeypatch):
    monkeypatch.setenv(
        "SAMUS_OUTREACH_CONVERSIONS_LOG",
        str(tmp_path / "conv.jsonl"),
    )
    monkeypatch.delenv("SAMUS_LEDGER_BACKEND", raising=False)
...
def test_admin_journey_returns_chronological_events(gateway_client):
    be.emit_business_event(be.LEAD_ENRICHED, workcell="prospecting", prospect_id="p_j1")
    be.emit_business_event(be.EMAIL_SENT, workcell="outreach", prospect_id="p_j1", campaign_id="c1")
    be.emit_business_event(be.EMAIL_SENT, workcell="outreach", prospect_id="p_other")
