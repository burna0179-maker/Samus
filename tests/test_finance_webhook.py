...
def _checkout_session_event(*, event_id: str = "evt_test_1",
...
def test_append_then_load_seen_event_ids(tmp_path, monkeypatch):
    ...
    append_event_record(WebhookEventRecord(
        event_id="evt_a", event_type="x", received_at=now_iso,
        livemode=False, process_status="processed",
    ))
    append_event_record(WebhookEventRecord(
        event_id="evt_b", event_type="y", received_at=now_iso,
        livemode=False, process_status="ignored",
    ))
...
def test_load_recent_events_filters_by_window(tmp_path, monkeypatch):
    ...
    append_event_record(WebhookEventRecord(
        event_id="old", event_type="checkout.session.completed",
        received_at=old_ts, livemode=False, process_status="processed",
    ))
    append_event_record(WebhookEventRecord(
        event_id="new", event_type="checkout.session.completed",
        received_at=new_ts, livemode=False, process_status="processed",
    ))
...
def test_event_ledger_rotation_drops_aged_rows(tmp_path, monkeypatch):
    ...
    aged = (datetime.now(timezone.utc) - timedelta(days=45)).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
    fresh = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

    append_event_record(WebhookEventRecord(
        event_id="evt_aged", event_type="x", received_at=aged,
        livemode=False, process_status="processed",
    ))
    append_event_record(WebhookEventRecord(
        event_id="evt_fresh", event_type="y", received_at=fresh,
        livemode=False, process_status="processed",
    ))
...
def test_handle_existing_paid_customer_no_op_transition(tmp_path, monkeypatch):
    ...
    cust = _FakeCustomer(id_="cust_bob", email="bob@example.com",
                         current_state="paid")
...
def test_handle_unpaid_does_not_advance_but_records_event(tmp_path, monkeypatch):
    ...
    event = _checkout_session_event(
        event_id="evt_unpaid", email="ach@example.com", payment_status="unpaid",
    )
    body = json.dumps(event).encode("utf-8")
    header = _sign(body, "whsec_test")
...
def test_handle_paid_advances_to_paid(tmp_path, monkeypatch):
    ...
    event = _checkout_session_event(
        event_id="evt_paid", email="card@example.com", payment_status="paid",
    )
    body = json.dumps(event).encode("utf-8")
    header = _sign(body, "whsec_test")
...
def test_handle_no_payment_required_advances_to_paid(tmp_path, monkeypatch):
    ...
    event = _checkout_session_event(
        event_id="evt_free", email="free@example.com",
...
def test_handle_unknown_event_type_returns_ignored(tmp_path, monkeypatch):
    ...
    _override_settings(monkeypatch)
    store = _FakeStore()
    event = {"id": "evt_unh", "type": "customer.subscription.updated",
...
def test_handle_invoice_paid_records_renewal_and_emits_payment(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(be, "emit_business_event",
                        lambda *a, **k: emitted.append((a, k)) or {})
    store = _FakeStore()
    event = {
        "id": "evt_renew1", "type": "invoice.paid", "livemode": True,
        "data": {"object": {
            "object": "invoice",
    ...
    lines = (tmp_path / "stripe_events.jsonl").read_text(
...
def test_handle_invoice_payment_succeeded_also_renews(tmp_path, monkeypatch):
    ...
    store = _FakeStore()
    event = {
        "id": "evt_renew2", "type": "invoice.payment_succeeded", "livemode": True,
        "data": {"object": {
            "object": "invoice", "customer_email": "k@x.com",
            "amount_paid": 30000, "currency": "usd",
...
def test_handle_checkout_session_expired_records_funnel_signal(tmp_path, monkeypatch):
    ...
    store = _FakeStore()
    event = _checkout_session_event(
        event_id="evt_exp1", email="alice@example.com", offer="seo_audit",
...
def test_handle_checkout_session_created_records_funnel_signal(tmp_path, monkeypatch):
    ...
    store = _FakeStore()
    event = _checkout_session_event(
        event_id="evt_crd1", email="", offer="seo_audit",
...
def test_get_recent_payments_rolls_up_counts(tmp_path, monkeypatch):
    ...
    append_event_record(WebhookEventRecord(
        event_id="p1", event_type="checkout.session.completed",
        received_at=fresh, livemode=True,
        customer_email="a@x.com", amount_total_usd=149.0,
        hf_offer_code="seo_audit", process_status="processed",
    ))
    append_event_record(WebhookEventRecord(
        event_id="p2", event_type="customer.subscription.updated",
        received_at=fresh, livemode=True, process_status="ignored",
    ))
    append_event_record(WebhookEventRecord(
        event_id="p3", event_type="checkout.session.completed",
        received_at=fresh, livemode=True, process_status="failed",
        process_note="no email",
    ))
    append_event_record(WebhookEventRecord(
        event_id="p_old", event_type="checkout.session.completed",
        received_at=stale, livemode=True, process_status="processed",
    ))
    from backend.finance.service import get_recent_payments
    rs = get_recent_payments(window_days=7)
...
def test_endpoint_200_on_unknown_event_type(tmp_path, monkeypatch):
    ...
    from backend.finance.app import app
    client = TestClient(app)
    body = json.dumps({"id": "evt_x", "type": "customer.created",
                       "livemode": True, "data": {"object": {}}}).encode()
    header = _sign(body, "whsec_test")
    r = client.post("/stripe_webhook", content=body,
...
def test_run_auto_fulfill_sync_appends_success_followup(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import (
        _run_auto_fulfill_sync, load_recent_events,
    )
...
def test_run_auto_fulfill_sync_appends_failure_followup(tmp_path, monkeypatch):
    ...
    rows = [
        e for e in load_recent_events(window_days=7)
        if e.event_id == "evt_followup_fail"
    ]
    assert len(rows) == 1
    row = rows[0]
    assert row.event_type == "auto_fulfill"
...
def test_run_auto_fulfill_sync_handles_crash(tmp_path, monkeypatch):
    ...
    rows = [
        e for e in load_recent_events(window_days=7)
        if e.event_id == "evt_followup_crash"
    ]
    assert len(rows) == 1
...
def test_handle_fires_close_the_loop_to_crm_on_success(tmp_path, monkeypatch):
    ...
    import backend.finance.webhook as web
    monkeypatch.setattr(
        web, "_dispatch_close_the_loop_to_crm",
...
def test_dispatch_helper_skips_when_gateway_url_unset(tmp_path, monkeypatch):
    ...
    fired: list = []
    monkeypatch.setattr(web, "signed_post_json_sync",
    ...
    _dispatch_close_the_loop_to_crm(
        email="x@x.com", amount_usd=100.0, event_id="evt_drop",
    )
    assert fired == []
...
def test_dispatch_helper_skips_when_hmac_key_unset(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import _dispatch_close_the_loop_to_crm
    _dispatch_close_the_loop_to_crm(
        email="x@x.com", amount_usd=50.0, event_id="evt_drop_hmac",
    )
    assert fired == []
...
def test_dispatch_helper_failure_does_not_propagate(tmp_path, monkeypatch):
    ...
    from backend.finance.webhook import _dispatch_close_the_loop_to_crm
    _dispatch_close_the_loop_to_crm(
        email="x@x.com", amount_usd=99.0, event_id="evt_thread_boom",
    )
...
def test_dispatch_helper_enqueues_close_payment_to_opportunity(
    ...
    from backend.finance.webhook import _dispatch_close_the_loop_to_crm
    _dispatch_close_the_loop_to_crm(
        email="buyer@x.com", amount_usd=1500.0, event_id="evt_test_001",
    )
...
def test_handle_fires_exact_opportunity_close_on_op_client_reference(
    ...
    import backend.finance.webhook as web
    monkeypatch.setattr(web, "_dispatch_close_opportunity_by_id",
                        lambda **kw: exact.append(kw))
    monkeypatch.setattr(web, "_dispatch_close_the_loop_to_crm",
                        lambda **kw: email_loop.append(kw))
...
def test_dispatch_close_opportunity_by_id_enqueues_correct_envelope(
    ...
    _dispatch_close_opportunity_by_id(
        opportunity_id="op_abc123", amount_usd=149.0, event_id="evt_x1",
    )
...
def test_dispatch_close_opportunity_by_id_skips_when_id_empty(
    ...
    from backend.finance.webhook import _dispatch_close_opportunity_by_id
    _dispatch_close_opportunity_by_id(
        opportunity_id="", amount_usd=10.0, event_id="evt_empty",
    )
    assert fired == []
...
def _override_settings_with_env(monkeypatch, *, env: str,
...
def test_get_recent_payments_correlates_auto_fulfill_followups(
    ...
    # Original payments
    for ev_id, email in [("p_ok", "ok@x.com"), ("p_fail", "fail@x.com"),
                          ("p_manual", "manual@x.com")]:
        append_event_record(WebhookEventRecord(
            event_id=ev_id, event_type="checkout.session.completed",
            received_at=fresh, livemode=True,
            customer_email=email, amount_total_usd=149.0,
            hf_offer_code="seo_audit", process_status="processed",
            auto_fulfill_attempted=(ev_id != "p_manual"),
        ))
    # Auto-fulfill follow-ups: one ok, one failed, p_manual has none
    append_event_record(WebhookEventRecord(
        event_id="p_ok", event_type="auto_fulfill", received_at=fresh,
        livemode=False, process_status="processed",
        auto_fulfill_attempted=True, auto_fulfill_ok=True,
        auto_fulfill_message_id="msg_ok",
    ))
    append_event_record(WebhookEventRecord(
        event_id="p_fail", event_type="auto_fulfill", received_at=fresh,
        livemode=False, process_status="failed",
        auto_fulfill_attempted=True, auto_fulfill_ok=False,
...
def test_out_ref_credits_outreach_prospect(monkeypatch, tmp_path):
    ...
    from backend.finance.webhook import handle_stripe_webhook
    event = _checkout_session_event(
        event_id="evt_flyer_1", email="buyer@acme.com", offer="workflow_rescue",
        amount_cents=50000, client_reference_id="out_pr_flyer42",
    )
    body = json.dumps(event).encode("utf-8")
    header = _sign(body, "whsec_test")
    result = handle_stripe_webhook(body, header, customer_store=_FakeStore())
