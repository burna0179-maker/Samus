...
def test_full_cost_sums_all_components():
    ...
    got = full_cost_usd(
        {"token_cost_usd": 0.30}, voice_cost_usd=0.45, email_sends=3,
    )
...
def test_compute_priority_hand_computed():
    ...
    got = compute_priority(
        ev_usd=1000.0, probability=0.5, urgency=0.8,
        time_estimate_hrs=2.0, cost_usd=0.5,
    )
...
def test_compute_priority_cost_floor_prevents_divide_blowup():
    ...
    got = compute_priority(
        ev_usd=100.0, probability=1.0, urgency=1.0,
        time_estimate_hrs=1.0, cost_usd=0.0,
    )
    assert got == pytest.approx(100.0 / (1.0 * PRIORITY_COST_FLOOR_USD))


def test_compute_priority_clamps_probability_and_urgency():
    a = compute_priority(ev_usd=100, probability=5.0, urgency=3.0,
                         time_estimate_hrs=1.0, cost_usd=1.0)
    b = compute_priority(ev_usd=100, probability=1.0, urgency=1.0,
                         time_estimate_hrs=1.0, cost_usd=1.0)
    assert a == b
...
def test_priority_score_hand_computed_fixture():
    score = priority_score(
        _opp(), now=_NOW, voice_cost_usd=0.0, email_sends=0, bandit_trials=15,
    )
...
def test_priority_score_email_and_voice_costs_flow_into_denominator():
    score = priority_score(
        _opp(token_cost_usd=0.0), now=_NOW,
        voice_cost_usd=0.98, email_sends=1, bandit_trials=0,
    )
...
def test_priority_score_unknown_stage_uses_default_time_estimate():
    score = priority_score(
        _opp(stage="weird"), now=_NOW, voice_cost_usd=0.0, bandit_trials=0,
    )
...
def test_priority_score_to_dict_round_trip():
    ...
    assert set(d) == {
        "ev_usd", "probability", "urgency", "time_estimate_hrs",
        "cost_usd", "confidence", "priority",
    }
