...
def _wf():
    return compile_workflow(TaskPlan(triggers=["form_submission"], actions=["post_to_slack"],
                                     notifications=[], tools=["slack"]), name="Deploy")


def _settings(**over):
    base = dict(workflow_n8n_deploy_enabled=False, workflow_n8n_dry_run=True,
...
def test_deploy_disabled_is_dormant(monkeypatch):
    called = {"n": 0}
    monkeypatch.setattr(deploy_mod, "_http_post", lambda *a, **k: called.__setitem__("n", called["n"] + 1))
...
def test_deploy_dry_run_makes_no_call(monkeypatch):
    called = {"n": 0}
    monkeypatch.setattr(deploy_mod, "_http_post", lambda *a, **k: called.__setitem__("n", called["n"] + 1))
    res = deploy_workflow(_wf(), settings=_settings(workflow_n8n_deploy_enabled=True, workflow_n8n_dry_run=True))
    assert res["status"] == "dry_run"
    assert called["n"] == 0


def test_deploy_armed_without_creds_fails_closed():
    res = deploy_workflow(_wf(), settings=_settings(
        workflow_n8n_deploy_enabled=True, workflow_n8n_dry_run=False, n8n_base_url="", n8n_api_key=""))
...
def test_deploy_live_posts_and_returns_id(monkeypatch):
    ...
    res = deploy_workflow(_wf(), settings=_settings(
        workflow_n8n_deploy_enabled=True, workflow_n8n_dry_run=False,
        n8n_base_url="https://n8n.example.com/", n8n_api_key="k"))
