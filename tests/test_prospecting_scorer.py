...
def test_score_rises_with_review_volume_log_scaled():
    ...
    assert (score_prospect(many) > score_prospect(some)
            > score_prospect(few) > score_prospect(none))
...
def test_score_spreads_no_52_ceiling():
    ...
    from backend.prospecting.scorer import score_prospect
    a = _record(industry="dentist", review_rating="4.9",
                review_count="300", seo_score=30)
    b = _record(industry="dentist", review_rating="4.1",
                review_count="300", seo_score=30)
...
def test_score_bounded_0_to_100():
    from backend.prospecting.scorer import score_prospect
    maxed = _record(industry="dentist", review_rating="5.0",
                    review_count="999", seo_score=0)
    floor = _record(industry="taxidermy", review_rating="0",
                    review_count="0", seo_score=100)
...
def test_access_blocked_site_gets_neutral_seo_credit():
    ...
    from backend.prospecting.scorer import score_prospect
    blocked = _record(industry="dentist", seo_score=0,
...
def test_all_soft_failure_statuses_get_neutral_seo_credit():
    ...
    from backend.prospecting.scorer import score_prospect
    base = score_prospect(_record(industry="dentist", seo_score=0,
                                  website_status="access_blocked"))
    for status in ("unreachable_timeout", "unreachable", "server_error",
                   "http_error", "empty"):
        same = score_prospect(_record(industry="dentist", seo_score=0,
...
def test_genuine_no_website_keeps_full_seo_opportunity():
    ...
    from backend.prospecting.scorer import score_prospect
    soft_fail = score_prospect(_record(industry="dentist", seo_score=0,
                                       website_status="unreachable_timeout"))
    for status in ("no_website", "parked", "social_only",
                   "domain_unresolved", "gone"):
        genuine = score_prospect(_record(industry="dentist", seo_score=0,
                                         website_status=status))
...
def test_measured_seo_score_unaffected_by_status():
    ...
    from backend.prospecting.scorer import score_prospect
    live = score_prospect(_record(industry="dentist", seo_score=40,
                                  website_status="live"))
    blocked = score_prospect(_record(industry="dentist", seo_score=40,
                                     website_status="access_blocked"))
    assert live == blocked
