...
def test_each_entry_action_key_matches_action_field():
    ...
    for key, schema in GROWTH_SCHEMA_REGISTRY.items():
        assert schema.action == key, (
            f"Registry key {key!r} != schema.action {schema.action!r}"
        )
...
def test_all_required_inputs_non_empty():
    ...
    for action, schema in GROWTH_SCHEMA_REGISTRY.items():
        assert schema.required_inputs, (
            f"Action {action!r} has no required_inputs"
        )
...
def test_all_output_fields_non_empty():
    ...
    for action, schema in GROWTH_SCHEMA_REGISTRY.items():
        assert schema.output_fields, (
            f"Action {action!r} has no output_fields"
        )
...
@pytest.mark.parametrize("action,complete_payload,missing_one", [
    (
        "geo_format",
...
def test_validate_catches_missing_required_field(action, complete_payload, missing_one):
    ...
    assert len(missing) >= 1, (
        f"Expected missing fields for {action!r} with incomplete payload"
    )
