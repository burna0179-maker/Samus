...
def _write(records: list[dict[str, Any]]) -> bool:
    ...
    try:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=os.path.dirname(path) or ".", prefix=".onc_", suffix=".tmp"
        )
        try:
...
def _iter_engagement_records(
    *, since: datetime, prospect_id: str,
...
def _summarise_signals(
    *, prospect_id: str, sent_at: datetime,
) -> dict[str, str | None]:
    ...
    *,
    ...
    for rec in records:
        if (
            ...
            return {"registered": False, "reason": "already_watching",
...
def mark_closed(
    *, prospect_id: str, reason: str, message_id: str = "",
) -> int:
...
def tick(
    *,
    ...
    nudge_enabled = bool(
        getattr(settings, "outreach_open_no_click_nudge_enabled", False)
    )
    if dry_run is None:
    ...
    for rec in records:
        ...
        try:
            sent_at = datetime.strptime(
                str(rec.get("sent_at") or ""), "%Y-%m-%dT%H:%M:%SZ",
            ).replace(tzinfo=timezone.utc)
        except ValueError:
            continue
        signals = _summarise_signals(
            prospect_id=str(rec.get("prospect_id") or ""), sent_at=sent_at,
        )
        ...
        first_open_dt = datetime.strptime(
            rec["first_open_at"], "%Y-%m-%dT%H:%M:%SZ",
        ).replace(tzinfo=timezone.utc)
        ...
        if not live_fire:
            ...
            dirty = True
            out.append({"prospect_id": rec["prospect_id"],
                        "action": "would_nudge_flag_off"})
            continue
        try:
