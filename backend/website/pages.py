...
def _service_blurb(name: str, item: str) -> str:
    ...
    if "deep" in it:
        return (
            f"A top-to-bottom reset. {name} scrubs the spots routine cleaning misses - baseboards, "
    ...
    if "airbnb" in it or "turnover" in it or "bnb" in it:
        return (
            f"Fast, reliable turnovers between guests. {name} resets the property to hotel standard, "
            "restocks the essentials, and flags anything that needs your attention before the next check-in."
        )
    if "move" in it:
        return (
            f"Moving in or out? {name} handles the full move-out (or move-in) clean - inside cabinets, "
    ...
    if "commercial" in it:
        return (
            f"Clean, professional spaces your clients and team notice. {name} works around your hours "
            "with dependable commercial cleaning for offices and storefronts."
        )
    if "residential" in it or "house" in it or "home" in it:
        return (
            f"Your home, consistently spotless. {name} handles the regular upkeep so you get your "
            "weekends back, with the same trusted person every visit."
        )
    if "regular" in it or "recurring" in it or "weekly" in it:
        return (
            f"Set it and forget it. {name} keeps your space clean on a schedule that fits your life - "
            "weekly, biweekly, or monthly."
        )
    if "one" in it and "time" in it:
        return (
            f"Need it clean once, done right? {name} delivers a thorough one-time clean for move-outs, "
...
def _gallery_page(brief: Any, media: dict[str, str]) -> dict[str, Any]:
    ...
    imgs = [
        ...
        for s in (
            _safe_url(u)
            for u in (media.get("hero_image"), media.get("section_image"), media.get("og_image"))
        )
        if s
    ]
    tiles = "".join(
...
def _faq_page(brief: Any) -> dict[str, Any]:
    ...
    head = schema_builder.to_script_tag(
        schema_builder.faq_page(items)
...
def _reviews_page(brief: Any) -> dict[str, Any]:
    ...
    cta = (
        (
            f'<a href="{_esc(review_url)}" target="_blank" rel="noopener" '
            'class="inline-block rounded-full px-8 py-4 font-semibold text-slate-900" '
            'style="background:linear-gradient(90deg,var(--accent2),var(--accent))">Leave a Review</a>'
        )
        if review_url
        else (
...
def _careers_page(brief: Any) -> dict[str, Any]:
    ...
    phone_line = (
        (
            f'<p class="mt-4 text-slate-600">Prefer to call? Reach us at '
            f'<a href="{_esc(tel_href)}" '
            f'class="font-semibold text-[var(--accent)]">{_esc(phone)}</a>.</p>'
        )
        if tel_href
        else ""
    )
    ...
    if embed:
        ...
        apply_href = (
            _safe_url(f"mailto:{email}?subject=Cleaning%20Position%20Application") if email else ""
        )
        if apply_href:
            apply_btn = (
                f'<a href="{_esc(apply_href)}" class="inline-block rounded-full px-8 py-4 font-semibold '
    ...
    looks = [
        ...
        (
            "Detail-oriented",
            "You take pride in the spots others miss and leave every space better than you found it.",
        ),
        (
            "Respectful",
            "You treat clients' homes and businesses with care, honesty, and discretion.",
        ),
        (
            "Team player",
            "We are family owned and operated - we look out for each other and the people we serve.",
        ),
    ]
    cards = "".join(
        '<div class="reveal glass rounded-2xl bg-white/70 border border-slate-200/70 p-7 shadow-sm '
...
def _area_page(brief: Any, area: str) -> dict[str, Any]:
    ...
    return {
        "file": f"areas/{_slug(area)}.html",
        "title": f"{primary} in {loc} | {name}",
        "label": area,
        "content": content,
        "head": schema_builder.to_script_tag(lb),
        "area": True,
    }
...
def build_marketing_pages(
    brief: Any, *, media: dict[str, str] | None = None
) -> list[dict[str, Any]]:
