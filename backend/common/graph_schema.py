...
QUERY_ALLOWLIST: Final[dict[str, str]] = {
    "account_by_id": "MATCH (a:Account {account_id: $account_id}) RETURN a LIMIT 1",
    "contacts_for_account": "MATCH (a:Account {account_id: $account_id})-[:HAS_CONTACT]->(c:Contact) RETURN c",
    "prospects_for_zipcode": "MATCH (p:Prospect {zipcode: $zipcode}) RETURN p LIMIT $limit",
    "task_lineage": "MATCH (t:Task {task_id: $task_id})-[:EMITTED]->(e:AuditEvent) "
    "RETURN t, collect(e) AS events",
    "recent_audit_events_for_service": "MATCH (e:AuditEvent {service: $service}) "
