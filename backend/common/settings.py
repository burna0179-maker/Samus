...
_KNOWN_SERVICES: tuple[str, ...] = (
    ...
    "portfolio_controller", "entropy", "retainer",
)
...
def bootstrap_settings() -> Settings:
    ...
    for service in (
        "gateway", "leadgen", "prospecting", "scaffold", "fulfillment", "memory",
        "feedback", "outreach", "optimizer", "proposal", "seo", "finance", "voice",
        "intake", "crm", "strategy", "campaigns",
        "signal_filter", "path_optimizer", "template_recovery",
        "portfolio_controller", "entropy",
    ):
    ...
    for service in (
        "leadgen", "prospecting", "scaffold", "fulfillment", "feedback",
        "outreach", "optimizer", "proposal", "seo", "cash_engine",
    ):
    ...
    return Settings(
        ...
        security_sentinel_url=_env("SECURITY_SENTINEL_URL", "http://localhost:8434") or "http://localhost:8434",
        ...
        docuseal_service_agreement_template_id=_env(
            "DOCUSEAL_SERVICE_AGREEMENT_TEMPLATE_ID", ""
        )
        or "",
        ...
        cash_engine_queue_service=_env("SAMUS_CASH_ENGINE_QUEUE_SERVICE", "cash_engine") or "cash_engine",
        ...
        ddb_attribution_table=_env("DDB_ATTRIBUTION_TABLE", "samus_attribution_variants") or "samus_attribution_variants",
        ...
        autonomy_reinforcement_enabled=_env_bool(
            "SAMUS_AUTONOMY_REINFORCEMENT_ENABLED", False,
        ),
        autonomy_autotuner_enabled=_env_bool(
            "SAMUS_AUTONOMY_AUTOTUNER_ENABLED", False,
        ),
        autonomy_upgrade_enabled=_env_bool(
            "SAMUS_AUTONOMY_UPGRADE_ENABLED", False,
        ),
        ...
        governed_autonomous_dial_enabled=_env_bool(
            "SAMUS_GOVERNED_AUTONOMOUS_DIAL_ENABLED", False,
        ),
        ...
        idle_production_drive_enabled=_env_bool(
            "SAMUS_IDLE_PRODUCTION_DRIVE_ENABLED", False,
        ),
        ...
        autonomy_meta_enabled=_env_bool(
            "SAMUS_AUTONOMY_META_ENABLED", True,
        ),
        ...
        cognitive_act_proposals_enabled=_env_bool(
            "SAMUS_COGNITIVE_ACT_PROPOSALS_ENABLED", False,
        ),
        ...
        cognitive_loop_enabled=_env_bool(
            "SAMUS_COGNITIVE_LOOP_ENABLED", True,
        ),
        ...
        cognition_proposal_promotion_enabled=_env_bool(
            "SAMUS_COGNITION_PROPOSAL_PROMOTION_ENABLED", True,
        ),
        ...
        cognition_promotion_block_cooldown_sec=_env_int(
            "SAMUS_COGNITION_PROMOTION_BLOCK_COOLDOWN_SEC", 600,
        ),
        ...
        cognition_cadence_enabled=_env_bool(
            "SAMUS_COGNITION_CADENCE_ENABLED", True,
        ),
        cognition_cadence_interval_seconds=_env_int(
            "SAMUS_COGNITION_CADENCE_INTERVAL_SECONDS", 60,
        ),
        cognition_cadence_jitter_seconds=_env_int(
            "SAMUS_COGNITION_CADENCE_JITTER_SECONDS", 10,
        ),
        ...
        samus_reengagement_enabled=_env_bool(
            "SAMUS_REENGAGEMENT_ENABLED", False,
        ),
        samus_reengagement_cooldown_days=_env_int(
            "SAMUS_REENGAGEMENT_COOLDOWN_DAYS", 30,
        ),
        samus_reengagement_max_per_run=_env_int(
            "SAMUS_REENGAGEMENT_MAX_PER_RUN", 25,
        ),
        ...
        outreach_buying_signal_route_enabled=_env_bool(
            "OUTREACH_BUYING_SIGNAL_ROUTE_ENABLED", False,
        ),
        ...
        outreach_website_form_warm_enroll_enabled=_env_bool(
            "OUTREACH_WEBSITE_FORM_WARM_ENROLL_ENABLED", False,
        ),
        ...
        intake_telemetry_ingest_enabled=_env_bool(
            "SAMUS_TELEMETRY_INGEST_ENABLED", False,
        ),
        ...
        postcall_remediate_enabled=_env_bool(
            "SAMUS_POSTCALL_REMEDIATE_ENABLED", False,
        ),
        ...
        outreach_open_no_click_nudge_enabled=_env_bool(
            "OUTREACH_OPEN_NO_CLICK_NUDGE_ENABLED", False,
        ),
        outreach_open_no_click_dwell_hours=max(1, _env_int(
            "OUTREACH_OPEN_NO_CLICK_DWELL_HOURS", 24,
        )),
        # Send-lint mode. off | audit | enforce. Default off.
        outreach_send_lint_mode=(
            os.getenv("OUTREACH_SEND_LINT_MODE", "off") or "off"
        ).strip().lower(),
        samus_dialer_cooldown_days=max(0, min(14, _env_int(
            "SAMUS_DIALER_COOLDOWN_DAYS", 7,
        ))),
        samus_dialer_voicemail_exempt_hours=max(0, _env_int(
            "SAMUS_DIALER_VOICEMAIL_EXEMPT_HOURS", 36,
        )),
        samus_dialer_business_hours_gate=_env_bool(
            "SAMUS_DIALER_BUSINESS_HOURS_GATE", False,
        ),
        ...
        prospecting_in_stack_cadence_enabled=_env_bool(
            "SAMUS_PROSPECTING_IN_STACK_CADENCE_ENABLED", False,
        ),
        ...
        website_content_generation_enabled=_env_bool(
            "SAMUS_WEBSITE_CONTENT_GENERATION_ENABLED", True,
        ),
        website_content_model=_env(
            "SAMUS_WEBSITE_CONTENT_MODEL", "claude-haiku-4-5-20251001",
        ...
        website_design_intelligence_enabled=_env_bool("SAMUS_WEBSITE_DESIGN_INTELLIGENCE_ENABLED", True),
        ...
        website_media_image_model=_env(
            "SAMUS_WEBSITE_MEDIA_IMAGE_MODEL", "gemini-2.5-flash-image",
        ...
        website_media_video_model=_env(
            "SAMUS_WEBSITE_MEDIA_VIDEO_MODEL", "veo-3.1-fast-generate-preview",
        ) or "veo-3.1-fast-generate-preview",
        website_media_video_resolution=_env("SAMUS_WEBSITE_MEDIA_VIDEO_RESOLUTION", "720p") or "720p",
        ...
        social_reel_tts_voice=_env("SAMUS_SOCIAL_REEL_TTS_VOICE", "en-US-AriaNeural") or "en-US-AriaNeural",
        ...
        ddb_idempotency_table=_env("DDB_IDEMPOTENCY_TABLE", "samus_idempotency") or "samus_idempotency",
        ddb_suppression_table=_env("DDB_SUPPRESSION_TABLE", "samus_suppression") or "samus_suppression",
        ddb_recipient_index_table=_env("DDB_RECIPIENT_INDEX_TABLE", "samus_recipient_index") or "samus_recipient_index",
        ddb_feedback_table=_env("DDB_FEEDBACK_TABLE", "samus_feedback_events") or "samus_feedback_events",
        ddb_onboarding_leads_table=_env("DDB_ONBOARDING_LEADS_TABLE", "samus_onboarding_leads") or "samus_onboarding_leads",
        ddb_llm_budgets_table=_env("DDB_LLM_BUDGETS_TABLE", "samus_llm_budgets") or "samus_llm_budgets",
        ...
        ddb_conversations_table=_env("DDB_CONVERSATIONS_TABLE", "samus_conversations") or "samus_conversations",
        ddb_call_state_table=_env("DDB_CALL_STATE_TABLE", "samus_call-State") or "samus_call-State",
        ddb_opportunities_table=_env("DDB_OPPORTUNITIES_TABLE", "samus_opportunities") or "samus_opportunities",
        ddb_operator_tasks_table=_env("DDB_OPERATOR_TASKS_TABLE", "samus_operator_tasks") or "samus_operator_tasks",
        ...
        crm_hivemind_projection_enabled=_env_bool(
            "SAMUS_CRM_HIVEMIND_PROJECTION_ENABLED", True,
        ),
        ...
        vector_store_collection=_env(
            "SAMUS_VECTOR_STORE_COLLECTION", "samus_knowledge",
        ) or "samus_knowledge",
        vector_store_path=_env(
            "SAMUS_VECTOR_STORE_PATH", "/opt/samus/data/vectorstore",
        ...
        anthropic_api_key=_env("ANTHROPIC_API_KEY", "") or "",  # Unused — LM Studio backend needs no auth
        ...
        revenue_entity_default=_env("REVENUE_ENTITY_DEFAULT", "HustleForge LLC") or "HustleForge LLC",
        ...
        samus_voice_callback_number=(
            _env("SAMUS_VOICE_CALLBACK_NUMBER", "(530) 418-5105")
            or "(530) 418-5105"
        ),
        ...
        samus_voice_profile_context_enabled=_env_bool(
            "SAMUS_VOICE_PROFILE_CONTEXT", False,
        ),
        ...
        sendgrid_base_url=_env("SENDGRID_BASE_URL", "https://api.sendgrid.com") or "https://api.sendgrid.com",
        auto_fulfill_offer_codes=[
            x.strip() for x in (_env("SAMUS_AUTO_FULFILL_OFFERS", "") or "").split(",")
            if x.strip()
        ],
        ...
        gmail_inbox_ledger_path=_env(
            "SAMUS_GMAIL_INBOX_LEDGER", "/opt/samus/data/intake/inbound_email.jsonl",
        ...
        llm_cheap_model_pattern=_env(
            "SAMUS_LLM_CHEAP_MODEL_PATTERN", r"^claude-haiku-",
        ) or r"^claude-haiku-",
        llm_circuit_breaker_threshold=_env_int(
            "SAMUS_LLM_CIRCUIT_BREAKER_THRESHOLD", 10,
        ),
        llm_circuit_breaker_cooldown_sec=_env_int(
            "SAMUS_LLM_CIRCUIT_BREAKER_COOLDOWN_SEC", 300,
        ),
        ...
        seo_security_audit_enabled=_env_bool(
            "SAMUS_SEO_SECURITY_AUDIT_ENABLED", True,
        ),
        ...
        seo_audit_cache_enabled=_env_bool(
            "SAMUS_AUDIT_CACHE_ENABLED", False,
        ),
        # ==================================================================
        ...
        intake_rate_limit_global_per_hour=_env_int(
            "SAMUS_INTAKE_RATE_LIMIT_GLOBAL_PER_HOUR", 600,
        ),
        intake_rate_limit_ttl_grace_seconds=_env_int(
            "SAMUS_INTAKE_RATE_LIMIT_TTL_GRACE_SECONDS", 120,
        ),
        ...
        samus_agora_relief_forward_interval_sec=_env_int(
            "SAMUS_AGORA_RELIEF_FORWARD_INTERVAL_SEC", 1800,
        ),
