...
DEFAULT_SCENARIOS: list[Scenario] = [
    Scenario(
        name="cost_shock",
        description="LLM failures spike (cost/reliability shock)",
    ...
    Scenario(
        name="error_storm",
        description="retry + error velocity spike across the queue",
    ...
    Scenario(
        name="channel_degraded",
        description="a top channel's error rate collapses throughput",
    ...
    Scenario(
        name="calm_baseline",
        description="no stress — the loop must not overreact",
        entropy_inputs={},
        workcells=[{"workcell": "seo", "throughput_efficiency": 0.9}],
        expect_stable=True,
        expect_no_quota_cut=True,
    ),
]
...
def run_scenario(scenario: Scenario) -> dict[str, Any]:
    ...
    _check(
        "countermeasures_fired",
        len(entropy["countermeasures"]) > 0,
        scenario.expect_countermeasures,
    )
