...
def observe_node_duration(
    client_id: str, template_id: str, node: str, seconds: float
) -> None:
    _safe(
        lambda: SAMUS_CAMPAIGN_NODE_DURATION_SECONDS.labels(
            client_id, template_id, node
        ).observe(max(0.0, float(seconds)))
    )


def record_node_failure(
    client_id: str, template_id: str, node: str, reason: str
) -> None:
    _safe(
...
def set_kpi_value(client_id: str, campaign_id: str, kpi: str, value: float) -> None:
    _safe(
        lambda: SAMUS_CAMPAIGN_KPI_VALUE.labels(client_id, campaign_id, kpi).set(
            float(value)
        )
    )
...
def inc_artifacts(client_id: str, campaign_id: str, artifact_type: str, n: int = 1) -> None:
    _safe(
        lambda: SAMUS_CAMPAIGN_ARTIFACTS_TOTAL.labels(
            client_id, campaign_id, artifact_type
        ).inc(n)
    )


def set_approval_queue(
    client_id: str, campaign_id: str, severity: str, depth: int
) -> None:
    _safe(
        lambda: SAMUS_CAMPAIGN_APPROVAL_QUEUE_TOTAL.labels(
            client_id, campaign_id, severity
        ).set(max(0, int(depth)))
    )


def inc_conversion(
    client_id: str, campaign_id: str, event_type: str, n: int = 1
) -> None:
    _safe(
