...
def _spec(node_type: str, version: float, parameters: dict[str, Any], cred: str = "") -> Spec:
    return {"type": node_type, "type_version": version, "parameters": parameters, "credential_hint": cred}


# --- triggers --------------------------------------------------------------

def _webhook(path: str) -> Spec:
    return _spec("n8n-nodes-base.webhook", 2.0,
...
_TRIGGER_SPECS = {
    ...
    "payment_received": lambda: _spec(
        "n8n-nodes-base.stripeTrigger", 1.0,
        {"events": ["checkout.session.completed"]}, cred="Stripe API",
    ),
}
...
def _crm_spec(tools: list[str]) -> Spec:
    if "hubspot" in tools:
        return _spec("n8n-nodes-base.hubspot", 2.1, {"resource": "contact", "operation": "create"}, "HubSpot")
    if "salesforce" in tools:
        return _spec("n8n-nodes-base.salesforce", 1.0, {"resource": "lead", "operation": "create"}, "Salesforce")
    if "pipedrive" in tools:
        return _spec("n8n-nodes-base.pipedrive", 1.0, {"resource": "deal", "operation": "create"}, "Pipedrive")
    return _spec("n8n-nodes-base.httpRequest", 4.2,
                 {"method": "POST", "url": "", "sendBody": True}, "HTTP Header Auth (CRM)")
...
def _sheet_spec(tools: list[str]) -> Spec:
    ...
    if "notion" in tools:
        return _spec("n8n-nodes-base.notion", 2.2, {"resource": "databasePage", "operation": "create"}, "Notion API")
    return _spec("n8n-nodes-base.googleSheets", 4.5,
...
def _email_spec(tools: list[str]) -> Spec:
    if "gmail" in tools:
        return _spec("n8n-nodes-base.gmail", 2.1,
                     {"resource": "message", "operation": "send", "sendTo": "", "subject": "", "message": ""},
                     "Gmail OAuth2")
    return _spec("n8n-nodes-base.emailSend", 2.1,
                 {"fromEmail": "", "toEmail": "", "subject": "", "text": ""}, "SMTP")


def _slack_spec(_tools: list[str]) -> Spec:
    return _spec("n8n-nodes-base.slack", 2.2,
                 {"resource": "message", "operation": "post", "select": "channel",
                  "channelId": "", "text": ""}, "Slack API")
...
_ACTION_SPECS = {
    ...
    "send_sms": lambda _t: _spec("n8n-nodes-base.twilio", 1.0,
                                 {"resource": "sms", "operation": "send", "from": "", "to": "", "message": ""},
                                 "Twilio API"),
    "generate_invoice": lambda _t: _spec("n8n-nodes-base.httpRequest", 4.2,
                                         {"method": "POST", "url": "https://api.stripe.com/v1/invoices",
                                          "sendBody": True}, "Stripe API (HTTP Header Auth)"),
    "route_to_owner": lambda _t: _spec("n8n-nodes-base.set", 3.4,
                                       {"assignments": {"assignments": [
                                           {"name": "owner", "value": "", "type": "string"}]}}),
    "schedule_followup": lambda _t: _spec("n8n-nodes-base.wait", 1.1, {"amount": 1, "unit": "days"}),
}
...
_NOTIFICATION_SPECS = {
    ...
    "discord_webhook": lambda _t: _spec("n8n-nodes-base.discord", 2.0,
                                        {"resource": "message", "operation": "sendLegacy",
                                         "authentication": "webhook", "content": ""},
                                        "Discord Webhook"),
}
...
def _http_fallback(label: str) -> Spec:
    return _spec("n8n-nodes-base.httpRequest", 4.2,
                 {"method": "POST", "url": "", "sendBody": True}, f"HTTP ({label})")
