...
def validate_workflow(wf: N8nWorkflow) -> list[Issue]:
    ...
    if len(triggers) == 0:
        ...
        issues.append(Issue("error", "multiple_triggers",
    ...
    if len(names) != len(name_set):
        ...
        issues.append(Issue("error", "duplicate_node_name", f"duplicate node names: {', '.join(dupes)}"))

    # connection integrity
    for source, conn in wf.connections.items():
        if source not in name_set:
            issues.append(Issue("error", "dangling_source", f"connection from unknown node '{source}'"))
        for branch in conn.get("main", []):
            for link in branch:
                tgt = link.get("node")
                if tgt not in name_set:
                    issues.append(Issue("error", "dangling_target",
    ...
    if triggers:
        ...
        for n in wf.nodes:
            ...
            if n.name not in reachable and n.name not in error_branch:
                issues.append(Issue("warning", "orphan_node", f"node '{n.name}' is not reachable from the trigger"))

    # credential report (non-blocking)
    for n in wf.nodes:
        if n.credential_hint:
            issues.append(Issue("info", "credential_required", f"'{n.name}' needs credential: {n.credential_hint}"))
