...
def _suggest_on_page(issue_id: str, audit: AuditResult, kw: list[str]) -> tuple[str, str] | None:
    ...
    if issue_id == "missing_canonical":
        ...
        return ("canonical_link", f'<link rel="canonical" href="{audit.url}">')
    if issue_id == "missing_og_title":
        title = findings.get("title") or (primary_kw or industry).title()
        return ("og_title", f'<meta property="og:title" content="{title[:90]}">')
...
def _recommendation_for(
    issue_id: str, severity: str, category: str, audit: AuditResult, kw: list[str]
    ...
    mapping = {
        "missing_title": (
            "title",
            "Add a <title> tag with the primary keyword",
            "Title tags are the highest-impact on-page SEO signal.",
        ),
        "title_length": (
            "title",
            "Rewrite the <title> to fit 10-70 chars",
        ...
        "meta_description_length": (
            "meta_description",
            "Tighten the meta description to 50-160 chars",
        ...
        "multiple_h1": (
            "heading",
            "Reduce to one <h1>; demote the others to <h2>",
        ...
        "missing_local_signals": (
            "local",
            "Add visible phone number + street address",
            "Local pack ranking requires NAP signals on the page.",
        ),
        "mixed_content": (
            "technical",
            "Migrate HTTP sub-resources to HTTPS",
            "Mixed content blocks indexing and breaks the lock icon.",
        ),
        "noindex_directive": (
            "technical",
            "Remove the <meta robots noindex> tag if SEO is desired",
            "Noindex pages are excluded from search results entirely.",
        ),
        "fetch_failed": (
            "technical",
            "Resolve fetch failure (DNS / 5xx / timeout)",
        ...
        "pagespeed_lcp_poor": (
            "technical",
            "Reduce Largest Contentful Paint below 2500 ms (hero image is usually the culprit)",
        ...
        "pagespeed_cls_poor": (
            "technical",
            "Eliminate layout shift below 0.1 (size all images + ad slots explicitly)",
        ...
        "pagespeed_performance_needs_improvement": (
            "technical",
            "Lift mobile Performance into the green band (>=90) when convenient",
        ...
        "pagespeed_cls_needs_improvement": (
            "technical",
            "Reduce layout shift below 0.1 by reserving space for late-loading widgets",
        ...
        "missing_local_business_schema": (
            "local",
            "Add a LocalBusiness @type (or a specific subtype) to your JSON-LD",
    ...
    return OptimizationRecommendation(
        area=area, action=action, rationale=rationale, priority=priority
    )


def build_recommendations(
    audit: AuditResult, target_keywords: list[str]
) -> tuple[list[OptimizationRecommendation], dict[str, str]]:
    """Return ``(recommendations, on_page_changes)``."""
    recs: list[OptimizationRecommendation] = []
    on_page: dict[str, str] = {}
    for issue in audit.issues:
        recs.append(
            _recommendation_for(issue.id, issue.severity, issue.category, audit, target_keywords)
        )
