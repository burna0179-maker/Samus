...
def check_security_headers(headers: dict[str, str]) -> list[SecurityFinding]:
    ...
    if not hsts:
        findings.append(SecurityFinding(
            id="missing_strict_transport_security", severity="medium",
            category="headers", title="No HSTS (Strict-Transport-Security) header",
        ...
        if gaps:
            findings.append(SecurityFinding(
                id="weak_strict_transport_security", severity="low",
                category="headers",
    ...
    if not xfo and not has_frame_ancestors:
        findings.append(SecurityFinding(
            id="missing_clickjacking_protection", severity="medium",
            category="headers", title="No clickjacking protection",
    ...
    if xcto != "nosniff":
        findings.append(SecurityFinding(
            id="missing_x_content_type_options", severity="low",
            category="headers", title="No X-Content-Type-Options: nosniff",
            evidence=(
                f"X-Content-Type-Options: {xcto}" if xcto
    ...
    if not referrer:
        findings.append(SecurityFinding(
            ...
            category="headers", title="No Referrer-Policy header",
    ...
    if not permissions:
        findings.append(SecurityFinding(
            ...
            category="headers", title="No Permissions-Policy header",
...
def check_cookie_flags(
    ...
    if insecure:
        findings.append(SecurityFinding(
            id="insecure_cookie_flags", severity="medium",
            category="cookies", title="Cookies set without Secure/HttpOnly flags",
...
def check_mixed_content(
    page_url: str, http_resources: list[str],
) -> list[SecurityFinding]:
    ...
    if resources:
        findings.append(SecurityFinding(
            id="mixed_content_resources", severity="medium",
            category="cookies", title="Secure page loads insecure (http://) resources",
...
def check_tls_certificate(hostname: str) -> list[SecurityFinding]:
    ...
    if not hostname:
        return [SecurityFinding(
            id="tls_check_skipped", severity="info", category="tls",
    ...
    try:
        ...
        with socket.create_connection(
            (hostname, 443), timeout=_TLS_TIMEOUT_S,
        ) as sock:
        ...
        return [SecurityFinding(
            id="tls_handshake_failed", severity="info", category="tls",
    ...
    if not cert:
        ...
        return [SecurityFinding(
            id="tls_certificate_unverified", severity="high", category="tls",
    ...
    if expiry is None:
        findings.append(SecurityFinding(
            id="tls_certificate_unreadable_expiry", severity="info",
            category="tls", title="TLS certificate expiry could not be read",
    ...
    if days_remaining < 0:
        findings.append(SecurityFinding(
            id="tls_certificate_expired", severity="critical", category="tls",
            title="TLS certificate has EXPIRED",
            evidence=f"issuer={issuer}; expired {expiry_str} "
        ...
        findings.append(SecurityFinding(
            id="tls_certificate_expiring_soon", severity="high", category="tls",
...
def probe_email_auth(domain: str) -> list[SecurityFinding]:
    ...
    if not domain:
        return [SecurityFinding(
            id="email_auth_check_skipped", severity="info",
            category="email_auth", title="Email authentication not checked",
    ...
    if not _HAS_DNSPYTHON:
        return [SecurityFinding(
            id="email_auth_dns_unavailable", severity="info",
            category="email_auth", title="Email authentication could not be checked",
    ...
    if txt_apex is None:
        ...
        return [SecurityFinding(
            id="email_auth_dns_unresolved", severity="info",
            category="email_auth", title="Could not resolve DNS for the domain",
...
def _audit_spf(domain: str, txt_apex: list[str]) -> list[SecurityFinding]:
    ...
    if not spf:
        return [SecurityFinding(
            id="spf_missing", severity="medium", category="email_auth",
    ...
    if spf_low.rstrip().endswith("+all") or " +all" in spf_low:
        findings.append(SecurityFinding(
            id="spf_permissive_all", severity="high", category="email_auth",
        ...
        findings.append(SecurityFinding(
            id="spf_no_terminating_all", severity="low", category="email_auth",
        ...
        findings.append(SecurityFinding(
            id="spf_present", severity="info", category="email_auth",
...
def _audit_dmarc(domain: str) -> list[SecurityFinding]:
    ...
    if dmarc_records is None:
        return [SecurityFinding(
            id="dmarc_dns_unresolved", severity="info", category="email_auth",
    ...
    if not dmarc:
        return [SecurityFinding(
            id="dmarc_missing", severity="high", category="email_auth",
    ...
    if policy == "none":
        return [SecurityFinding(
            id="dmarc_policy_none", severity="medium", category="email_auth",
    ...
    return [SecurityFinding(
        id="dmarc_enforced", severity="info", category="email_auth",
...
def _audit_dkim(domain: str) -> list[SecurityFinding]:
    ...
    if found_selector:
        return [SecurityFinding(
            id="dkim_present", severity="info", category="email_auth",
            title="A DKIM signing key was found",
            evidence=f"DKIM record at {found_selector}._domainkey.{domain}",
            risk="",
            remediation=(
                "No action required. DKIM is signing your outbound mail."
            ),
    ...
    return [SecurityFinding(
        id="dkim_not_detected", severity="low", category="email_auth",
        title="No DKIM signing key detected on common selectors",
        evidence=(
            "no DKIM TXT record at the common selectors checked "
            f"({', '.join(_DKIM_SELECTORS)})"
        ),
        risk=(
...
def _audit_caa(domain: str) -> list[SecurityFinding]:
    try:
        ...
        return [SecurityFinding(
            id="caa_dns_unresolved", severity="info", category="email_auth",
    ...
    if has_caa:
        return [SecurityFinding(
            id="caa_present", severity="info", category="email_auth",
...
def check_platform_exposure(
    base_origin: str, html: str, headers: dict[str, str], budget: _ProbeBudget,
) -> list[SecurityFinding]:
    ...
    findings.append(SecurityFinding(
        id="platform_wordpress_detected", severity="info", category="platform",
    ...
    if users_resp is not None and users_resp.status_code == 200:
        ...
        if names:
            findings.append(SecurityFinding(
                id="wordpress_user_enumeration", severity="medium",
                category="platform", title="WordPress leaks usernames via the REST API",
    ...
    if xmlrpc_resp is not None and xmlrpc_resp.status_code in (200, 405):
        ...
        if xmlrpc_resp.status_code == 405 or "xml-rpc server" in body_low:
            findings.append(SecurityFinding(
                id="wordpress_xmlrpc_enabled", severity="medium",
                category="platform", title="WordPress XML-RPC interface is enabled",
    ...
    if readme_resp is not None and readme_resp.status_code == 200 \
        ...
        findings.append(SecurityFinding(
            id="wordpress_readme_exposed", severity="low", category="platform",
...
def check_exposed_artifacts(
    base_origin: str, budget: _ProbeBudget,
) -> list[SecurityFinding]:
    ...
    if sec_txt is not None and sec_txt.status_code == 200 \
            and sec_txt.text.strip():
        findings.append(SecurityFinding(
            id="security_txt_present", severity="info", category="exposure",
        ...
        findings.append(SecurityFinding(
            id="security_txt_missing", severity="info", category="exposure",
    ...
    if git_cfg is not None and git_cfg.status_code == 200 \
            and "[core]" in (git_cfg.text or ""):
        findings.append(SecurityFinding(
            id="git_directory_exposed", severity="high", category="exposure",
...
def infrastructure_health(findings: list[SecurityFinding]) -> dict[str, Any]:
    ...
    try:
        infra = [
            ...
            if getattr(f, "category", "") in _INFRA_CATEGORIES
        ]
        score_100 = 100
        worst = ""
        ...
        return {
            "score": 1.0, "rating": "healthy",
            "infra_findings_considered": 0, "worst_severity": "",
        }
    ...
    *,
    ...
    def _run(name: str, fn) -> None:
        ...
        try:
            ...
            findings.append(SecurityFinding(
                id=f"{name}_check_error", severity="info", category="exposure",
